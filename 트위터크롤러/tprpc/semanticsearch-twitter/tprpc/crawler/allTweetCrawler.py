...
def checkKorean(data):
    cnt = 0
    for tweet in data:
        try:
            data = re.findall(u'[\uAC00-\uD7A3]+',unicode(tweet['tweet']))
#print data
            cnt += len(data)
        except:
            print "korean check error"
    return cnt
...
#time.sleep(10)
        ...
        # 한글 체크
        koreanCnt = checkKorean(data['crawlTweets'])
        if not koreanCnt == 0:
            print koreanCnt,
    ...
    try:
        ...
        query = "UPDATE `tweeter_pool` SET `latest_time` = '%d', `weight` = '%f', `nCrawl_time` = DATE_ADD(now(), INTERVAL +%d hour) WHERE `twit_id` = '%s'"%(maxTimestamp, nextWeight, nextTime ,twitId)
        print query
        cursor.execute(query)
        db.commit()
    except:
